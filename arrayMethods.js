...
// CODE HERE
const postTaxPrices = prices.map(p => (p * 1.07).toFixed(2));
...
const orderTotals = orders.map(o => o.price + (o.price * o.tax)).reduce((acc, cur) => acc + cur);
...
const bobsTotal = purchases.filter(p => p.owner === "Bob").reduce((arr, cur) => arr + cur.price, 0);
